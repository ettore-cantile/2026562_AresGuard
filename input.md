...
| **US04** | Mission Specialist | **As a** Mission Specialist, **I want to** clearly see the active or inactive status of the habitat heater **so that** I can confirm whether a manual or automatic command was successfully executed. | **AC:** The dashboard must accurately reflect the real-time ON/OFF state of the heater. <br>**NFR:** Actuator states must be fetched from the API and rendered on the frontend. |
